...
| 📚 **RAG 知识增强** | 6 城手工知识库（165 景点/108 美食/77 酒店），BGE 向量 + FAISS + Rerank 精排，方案推荐**强制引用知识库 source_id**，可追溯不胡编 |
...
1. 克隆项目后，**双击 `start.bat`**（首次运行会自动：安装依赖 → 下载模型 → 构建索引 → 启动前后端 → 打开浏览器）
2. 首次运行前，先编辑 `backend/.env` 填入 **DEEPSEEK_API_KEY**（https://platform.deepseek.com 注册，新用户有免费额度）
3. 打开 http://localhost:5173 即可使用

> 脚本会自动检查：Python → 依赖 → 模型（缺失自动下载约 1.5GB）→ 向量索引 → 前端依赖 → 端口占用，全部就绪后启动服务。关闭两个日志窗口即停止服务。
...
> **定位声明**：精选 6 城作为验证场景（覆盖多城联游 / 长途分段 / 单城深度），项目重点在**知识库构建流水线与质量保障机制**：覆盖矩阵设计 → LLM 辅助生成 → 规则校验 → 高德 POI 坐标校正 → 人工抽检。新增城市通过 `gen_city_kb.py` + `verify_knowledge.py` 流水线当天完成，可低成本扩展。
